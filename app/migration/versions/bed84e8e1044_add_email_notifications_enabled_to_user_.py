...
def upgrade() -> None:
    ...
    def safe_drop_index(index_name, table_name):
        try:
            ...
            pass  # Игнорируем ошибки, если индекс не существует
    
    # Удаляем индексы с проверкой существования
    safe_drop_index('ix_exercise_image_id', 'exercise')
    safe_drop_index('ix_exercise_reference_id', 'exercise')
    safe_drop_index('ix_exercise_type', 'exercise')
    safe_drop_index('ix_exercise_user_id', 'exercise')
    safe_drop_index('ix_exercise_video_id', 'exercise')
    safe_drop_index('ix_exercise_video_preview_id', 'exercise')
    safe_drop_index('ix_exercise_reference_exercise_type', 'exercise_reference')
    safe_drop_index('ix_exercise_reference_gif_id', 'exercise_reference')
    safe_drop_index('ix_exercise_reference_image_id', 'exercise_reference')
    safe_drop_index('ix_exercise_reference_user_id', 'exercise_reference')
    safe_drop_index('ix_exercise_reference_video_id', 'exercise_reference')
    safe_drop_index('ix_program_category_id', 'program')
    safe_drop_index('ix_program_image_id', 'program')
    safe_drop_index('ix_program_type_actual', 'program')
    safe_drop_index('ix_program_user_id', 'program')
    
    op.alter_column('training', 'muscle_group',
               existing_type=sa.VARCHAR(),
               nullable=False)
    
    safe_drop_index('ix_training_image_id', 'training')
    safe_drop_index('ix_training_program_id', 'training')
    safe_drop_index('ix_training_type_actual', 'training')
    safe_drop_index('ix_training_user_id', 'training')
    
    op.add_column('user', sa.Column('email_notifications_enabled', sa.Boolean(), nullable=True))
    
    safe_drop_index('ix_user_exercise_exercise_id', 'user_exercise')
    safe_drop_index('ix_user_exercise_program_id', 'user_exercise')
    safe_drop_index('ix_user_exercise_status', 'user_exercise')
    safe_drop_index('ix_user_exercise_training_date', 'user_exercise')
    safe_drop_index('ix_user_exercise_training_id', 'user_exercise')
    safe_drop_index('ix_user_exercise_user_id', 'user_exercise')
    safe_drop_index('ix_user_training_program_id', 'user_training')
    safe_drop_index('ix_user_training_status', 'user_training')
    safe_drop_index('ix_user_training_training_date', 'user_training')
    safe_drop_index('ix_user_training_training_id', 'user_training')
    safe_drop_index('ix_user_training_user_id', 'user_training')
